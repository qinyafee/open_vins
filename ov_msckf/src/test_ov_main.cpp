...
int main(int argc, char **argv)
{
    ...
    std::string basePath = "D:/record_data/20210517.tar/20210517";

	configPath = "D:/project/OpenVinsQinYaFei/out/build/1WMGH521JY9194/stereo_imu_ov.yaml";
	vins_configPath = "D:/project/OpenVinsQinYaFei/out/build/1WMGH521JY9194/stereo_imu.yaml";
	basePath = "D:/record_data/rfts_yjd_data_hmd_41_jitter";
    ...
    params = parse_ov(configPath);
    InitSystem(vins_configPath);
    size_t num = 5;
        ...
        while (getline(f_seq, line_seq))
        {
            ...
            if (dataType == 13 || dataType == 3)
            {
				for (size_t i = 0; i < num; i++)
				{
					std::string imgPath;
					char buf[50];
					snprintf(buf, sizeof(buf), "/images/%d/%014lld.jpg", i, timestamp);
                    imgPath = buf;

					imgs_str_buffer[timestamp].push_back(basePath + imgPath);
				}
				data_sequence[timestamp].push_back(Camera_Sensor);
            }
        }
    }
