...
#if HAVE_CUDA
void TrackKLT::feed_monocular_cuda(double timestamp, cv::Mat &img, size_t cam_id) {
  ...
  if (pts_last[cam_id].empty()) {
    // Detect new features
    perform_detection_monocular(img, gpu_img, pts_last[cam_id], ids_last[cam_id]);
    // Save the current image and pyramid
    img_last[cam_id] = img.clone();
    gpu_img_last[cam_id] = gpu_img;
    return;
  }

  // First we should make that the last images have enough features so we can do KLT
  // This will "top-off" our number of tracks so always have a constant number
  perform_detection_monocular(img_last[cam_id], gpu_img_last[cam_id], pts_last[cam_id], ids_last[cam_id]);
  rT3 = boost::posix_time::microsec_clock::local_time();
  ...
  // Lets track temporally
  perform_matching(gpu_img_last[cam_id], gpu_img, pts_last[cam_id], pts_left_new, cam_id, cam_id, mask_ll);
  assert(pts_left_new.size()==ids_last[cam_id].size());
  rT4 = boost::posix_time::microsec_clock::local_time();
...
void TrackKLT::feed_stereo_cuda(double timestamp, cv::Mat &img_leftin, cv::Mat &img_rightin, size_t cam_id_left, size_t cam_id_right) {
  ...
  cv::cuda::GpuMat gpu_img_left(img_left);
  cv::cuda::GpuMat gpu_img_right(img_right);
  rT2 = boost::posix_time::microsec_clock::local_time();
  ...
  if (pts_last[cam_id_left].empty() || pts_last[cam_id_right].empty()) {
    // Track into the new image
    perform_detection_stereo(img_left, img_right, gpu_img_left, gpu_img_right, pts_last[cam_id_left], pts_last[cam_id_right], 
                             ids_last[cam_id_left], ids_last[cam_id_right]);
    // Save the current image and pyramid
    img_last[cam_id_left] = img_left;
    img_last[cam_id_right] = img_right;
    gpu_img_last[cam_id_left] = gpu_img_left;
    gpu_img_last[cam_id_right] = gpu_img_right;
    return;
  }

  // First we should make that the last images have enough features so we can do KLT
  // This will "top-off" our number of tracks so always have a constant number
  perform_detection_stereo(img_last[cam_id_left], img_last[cam_id_right], gpu_img_last[cam_id_left], gpu_img_last[cam_id_right], 
  ...
  if(pts_last[cam_id_left].size() < min_matches || pts_last[cam_id_right].size() < min_matches){
    ...
    img_last[cam_id_left] = std::move(img_left);
    img_last[cam_id_right] = std::move(img_right);    
    gpu_img_last[cam_id_left] = std::move(gpu_img_left);
    gpu_img_last[cam_id_right] = std::move(gpu_img_right);
    pts_last[cam_id_left].clear();
    pts_last[cam_id_right].clear();
    ids_last[cam_id_left].clear();
    ids_last[cam_id_right].clear();
  ...
  parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
                  for (int i = range.start; i < range.end; i++) {
                    bool is_left = (i == 0);
                    perform_matching(gpu_img_last[is_left ? cam_id_left : cam_id_right], is_left ? gpu_img_left : gpu_img_right,
                                     pts_last[is_left ? cam_id_left : cam_id_right], is_left ? pts_left_new : pts_right_new,
                                     is_left ? cam_id_left : cam_id_right, is_left ? cam_id_left : cam_id_right,
                                     is_left ? mask_ll : mask_rr);
                  }
...
void TrackKLT::perform_detection_monocular(const cv::Mat &img0, const cv::cuda::GpuMat &gpu_img0, std::vector<cv::KeyPoint> &pts0,
                                           std::vector<size_t> &ids0) {
  ...
  // out-of-bounds
  Eigen::MatrixXi grid_2d =
      Eigen::MatrixXi::Zero((int)(img0.rows / min_px_dist) + 15, (int)(img0.cols / min_px_dist) + 15);
  auto it0 = pts0.begin();
  auto it2 = ids0.begin();
  while (it0 != pts0.end()) {
    // Get current left keypoint
  ...
  // Extract our features (use fast with griding)
  std::vector<cv::KeyPoint> pts0_ext;
  Grider_FAST::perform_griding(img0, gpu_img0, pts0_ext, num_featsneeded, grid_x, grid_y, threshold, true);
  ...
  for (auto &kpt : pts0_ext) {
    // Check that it is in bounds
    bool outof_bounds_0 = (kpt.pt.x < 0 || kpt.pt.y < 0 || kpt.pt.x >= img0.cols || kpt.pt.y >= img0.rows);
    if (outof_bounds_0)
      continue;
    // See if there is a point at this location
    if (grid_2d((int)(kpt.pt.y / min_px_dist), (int)(kpt.pt.x / min_px_dist)) == 1)
...
void TrackKLT::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1, const cv::cuda::GpuMat &gpu_img0, const cv::cuda::GpuMat &gpu_img1,
                                        std::vector<cv::KeyPoint> &pts0, std::vector<cv::KeyPoint> &pts1,
                                        std::vector<size_t> &ids0, std::vector<size_t> &ids1) {
  ...
  Eigen::MatrixXi grid_2d_0 =
      Eigen::MatrixXi::Zero((int)(img0.rows / min_px_dist) + 15, (int)(img0.cols / min_px_dist) + 15);
  Eigen::MatrixXi grid_2d_1 =
      Eigen::MatrixXi::Zero((int)(img1.rows / min_px_dist) + 15, (int)(img1.cols / min_px_dist) + 15);
  auto it0 = pts0.begin();
  auto it1 = ids0.begin();
  while (it0 != pts0.end()) {
    // Get current left keypoint
  ...
  if (num_featsneeded_0 > 0.2 * num_features) {
    ...
    // Extract our features (use fast with griding)
    std::vector<cv::KeyPoint> pts0_ext;
    Grider_FAST::perform_griding(img0, gpu_img0, pts0_ext, num_featsneeded_0, grid_x, grid_y, threshold, true);
    ...
    if (!pts0_new.empty()) {
      ...
      d_pyrLK_sparse->calc(gpu_img0, gpu_img1, prev_gpu_pts, cur_gpu_pts, gpu_status);
      
      std::vector<cv::Point2f> tmp_cur_pts(cur_gpu_pts.cols);
      cur_gpu_pts.download(tmp_cur_pts);
      pts1_new = std::move(tmp_cur_pts);

      std::vector<uchar> mask(gpu_status.cols);
      gpu_status.download(mask);

      // Loop through and record only ones that are valid
      for (size_t i = 0; i < pts0_new.size(); i++) {

        // Check that our tracks are in bounds
        bool outof_bounds_0 = (pts0_new.at(i).x < 0 || pts0_new.at(i).y < 0 || pts0_new.at(i).x >= img0.cols ||
                               pts0_new.at(i).y >= img0.rows);
        bool outof_bounds_1 = (pts1_new.at(i).x < 0 || pts1_new.at(i).y < 0 || pts1_new.at(i).x >= img1.cols ||
                               pts1_new.at(i).y >= img1.rows);
  ...
  if (num_featsneeded_1 > 0.2 * num_features) {
    ...
    // Extract our features (use fast with griding)
    std::vector<cv::KeyPoint> pts1_ext;
    Grider_FAST::perform_griding(img1, gpu_img1, pts1_ext, num_featsneeded_1, grid_x, grid_y, threshold, true);
...
void TrackKLT::perform_matching(const cv::cuda::GpuMat &gpu_img0, const cv::cuda::GpuMat &gpu_img1, std::vector<cv::KeyPoint> &kpts0,
                                std::vector<cv::KeyPoint> &kpts1, size_t id0, size_t id1, std::vector<uchar> &mask_out){
  ...
  cv::cuda::GpuMat prev_gpu_pts(pts0);
  cv::cuda::GpuMat cur_gpu_pts(pts1);
  cv::cuda::GpuMat gpu_status;
  ...
  std::vector<cv::Point2f> tmp_cur_pts(cur_gpu_pts.cols);
  cur_gpu_pts.download(tmp_cur_pts);
  pts1 = std::move(tmp_cur_pts);

  std::vector<uchar> mask_klt(gpu_status.cols);
  gpu_status.download(mask_klt);
